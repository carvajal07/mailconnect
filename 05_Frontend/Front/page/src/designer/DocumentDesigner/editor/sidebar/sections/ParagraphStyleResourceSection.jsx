...
import { ChevronRight, ChevronDown, Plus, Search, Pencil, PenLine, Copy, Trash2, MoreVertical, AlignLeft, Lock } from 'lucide-react';
import { ResourceItemMenu } from '../components/ResourceItemMenu.jsx';
import { UsageModal } from '../components/UsageModal.jsx';

export function ParagraphStyleResourceSection({ state, forceOpen, expandTick }) {
  ...
  const items = state.template?.styles?.paragraph ?? [];

  useEffect(() => {
    if (state.panelContext?.startsWith('paragraphStyle:')) setOpen(true);
  }, [state.panelContext]);
  ...
  const selectedId = state.panelContext?.startsWith('paragraphStyle:')
    ? state.panelContext.slice('paragraphStyle:'.length)
    : null;

  function handleAdd() {
    const id = state.addParagraphStyle?.();
    if (id) state.setPanelContext?.('paragraphStyle:' + id);
  }

  function handleSelect(id) { state.setPanelContext?.('paragraphStyle:' + id); }

  function handleRemove(id) {
    const usages = state.getParagraphStyleUsage?.(id) ?? [];
    if (usages.length > 0) { setUsageFor(id); return; }
    state.removeParagraphStyle?.(id);
  }

  function getUsageCount(id) { return (state.getParagraphStyleUsage?.(id) ?? []).length; }

  const usageItem = usageFor ? items.find(s => s.id === usageFor) : null;
  const usages    = usageFor ? (state.getParagraphStyleUsage?.(usageFor) ?? []) : [];
  ...
  function handleClone(id) {
    const newId = state.cloneParagraphStyle?.(id);
    if (newId) state.setPanelContext?.('paragraphStyle:' + newId);
  }

  function commitRename(id) {
    if (renamingName.trim()) state.updateParagraphStyle?.(id, { name: renamingName.trim() });
    setRenamingId(null);
  }
    ...
    <div className="dsb-resource">
      ...
      {open && (
        <div className="dsb-resource__body">
          {items.length > 0
            ...
            : <p className="dsb-resource__empty">Sin paragraph styles</p>
          }
        </div>
      )}
      {itemMenu && (
      ...
      {headerMenu && (
        <ResourceItemMenu
          ...
          actions={[{ label: 'Crear Paragraph Style', Icon: Plus, onClick: handleAdd }]}
      ...
      {usageFor && (
        <UsageModal
          areaId={usageFor}
          label={usageItem?.name || 'Paragraph Style'}
          usages={usages.map(u => ({
            ...u,
            elementId: u.elementId || u.areaId,
            pageName: u.pageName || (u.type === 'area' ? 'Content Area' : ''),
          }))}
