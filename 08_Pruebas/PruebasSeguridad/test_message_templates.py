...
def test_create_sms(mt):
    create, _, _ = mt
    resp = create.lambda_handler({'customerId': 'CU1', 'channel': 'SMS', 'name': 'Promo', 'body': 'Hola {{Nombre}}'}, None)
...
def test_create_sms_sin_body_400(mt):
    create, _, _ = mt
    assert create.lambda_handler({'customerId': 'CU1', 'channel': 'SMS', 'name': 'X'}, None)['statusCode'] == 400


def test_create_wsp_requiere_hsm(mt):
    create, _, _ = mt
    assert create.lambda_handler({'customerId': 'CU1', 'channel': 'WSP', 'name': 'X'}, None)['statusCode'] == 400
    ok = create.lambda_handler({'customerId': 'CU1', 'channel': 'WSP', 'name': 'Bienvenida',
                                'hsmName': 'welcome', 'language': 'es', 'params': ['Nombre']}, None)
    assert ok['statusCode'] == 201


def test_create_docx_requiere_s3path(mt):
    create, _, _ = mt
    assert create.lambda_handler({'customerId': 'CU1', 'channel': 'DOCX', 'name': 'X'}, None)['statusCode'] == 400
    ok = create.lambda_handler({'customerId': 'CU1', 'channel': 'DOCX', 'name': 'Carta',
                                's3Path': 'docs/carta.docx', 'params': ['Nombre', 'Valor']}, None)
    assert ok['statusCode'] == 201
...
def test_list_filtra_por_canal_y_tenant(mt):
    ...
    create.lambda_handler({'customerId': 'CU1', 'channel': 'WSP', 'name': 'B', 'hsmName': 'h'}, None)
    create.lambda_handler({'customerId': 'CU2', 'channel': 'SMS', 'name': 'C', 'body': 'x'}, None)

    # Todas las de CU1
    resp = lst.lambda_handler({'customerId': 'CU1'}, None)
    assert resp['data']['count'] == 2
    # Solo SMS de CU1
    resp_sms = lst.lambda_handler({'customerId': 'CU1', 'channel': 'SMS'}, None)
...
def test_delete_verifica_dueno(mt):
    create, _, delete = mt
    tid = create.lambda_handler({'customerId': 'CU1', 'channel': 'SMS', 'name': 'A', 'body': 'x'}, None)['data']['messageTemplateId']
...
def test_delete_inexistente_404(mt):
    _, _, delete = mt
    assert delete.lambda_handler({'messageTemplateId': 'nope'}, None)['statusCode'] == 404
