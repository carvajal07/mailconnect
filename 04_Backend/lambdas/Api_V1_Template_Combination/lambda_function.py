...
def send_sqs(url_sqs:str,message:dict)->None:
    """
    Esta función realiza el envio a las colas de SQS.

    Args:
        url_sqs (str): Url de la cola SQS en AWS
        message (dict): Lista con los mensajes (Maximo 10)
...
def download_attachments_data(campaign_id:str)->str:
    ...
    if items:
        for item in items:
            ...
            s3.download_file(bucket_name.lower(), attachment_path, template_temp_file)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        tenant = tenant_key(nit)    # llave de {tenant}_processDetail
