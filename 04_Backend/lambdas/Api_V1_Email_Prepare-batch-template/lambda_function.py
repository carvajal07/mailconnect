...
def notify_low_balance_if_needed(st, new_balance):
    ...
    try:
        ...
        # In-app: el correo se puede pasar por alto, la campanita del portal no.
        _notify_users(
            _tenant_users(st.customer_id, roles=('owner',)), 'balance.low',
            'Saldo bajo',
            'Tras el último envío tu saldo quedó en ${} COP, por debajo de tu umbral de '
            'aviso (${} COP).'.format(saldo, umbral),
            level='warning', link='saldo', customer_id=st.customer_id)
        ...
        html = brand_email(
            'Tu saldo está bajo',
            mail_p('Tras tu último envío, el saldo de <b>{company}</b> quedó por debajo de '
                   'tu umbral de aviso. Recarga para que tus próximas campañas no se '
                   'detengan por saldo insuficiente.'.format(
                       company=_mail_esc(st.customer_name or '')))
            + mail_rows([('Saldo actual', '${} COP'.format(saldo), '#e5484d'),
                         ('Tu umbral de aviso', '${} COP'.format(umbral), None)]),
            cta=('Recargar saldo', NOTIFY_DASHBOARD_URL),
            nota='Puedes ajustar el umbral o desactivar este aviso en el portal (Mi cuenta).',
            preheader='Saldo en ${} COP, por debajo de tu umbral.'.format(saldo))
