...
def send_recovery_email(email, code):
    ...
    html_body = brand_email(
        ...
        mail_p('Recibimos una solicitud para restablecer la contraseña de tu cuenta. '
               'Usa este código para continuar:')
        + mail_code(code),
        nota='El código es de un solo uso. Si no solicitaste este cambio, ignora este '
             'mensaje y tu contraseña seguirá igual.',
        preheader='Tu código para restablecer la contraseña es {}.'.format(code))
